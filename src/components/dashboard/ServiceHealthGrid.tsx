...
const META: Record<HealthStatus, { dot: string; label: string; text: string }> = {
  healthy: { dot: "bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.3)]", label: "Healthy", text: "text-emerald-300" },
  degraded: { dot: "bg-amber-400 shadow-[0_0_8px_rgba(251,191,36,0.3)]", label: "Degraded", text: "text-amber-300" },
  offline: { dot: "bg-red-400 shadow-[0_0_8px_rgba(239,68,68,0.3)]", label: "Offline", text: "text-red-300" },
  ...
  <div className="nexus-card-elevated p-6">
    <div className="flex items-center justify-between">
      <div>
        ...
        <h2 className="mt-2 text-2xl font-bold text-white">System status</h2>
      </div>
      <button
        type="button"
        onClick={onRefresh}
        className="nexus-button-glossy px-4 py-2.5 text-sm"
      >
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
      {services.map((service) => {
        ...
        return (
          <div
            key={service.key}
            className="flex items-center gap-3 rounded-2xl border border-white/[0.05] bg-slate-950/40 p-4 transition-all duration-200 hover:border-white/[0.08] hover:bg-slate-950/60"
          >
